...
- Публичный адрес работает по HTTPS; live-камера и микрофон доступны на домене.

## P0 — блокирует продовую эксплуатацию

- [x] **TLS и домен.** Домен `napoleonit-career.ru` указывает на сервер, сертификат Let’s Encrypt
  выпущен, настроен HTTP→HTTPS и открыт TCP/443. Сертификат покрывает только корневой домен.
