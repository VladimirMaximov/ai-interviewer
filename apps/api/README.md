...
PYTHONPATH=apps/api python apps/api/scripts/create_demo_invitation.py
```

Install backend dependencies once with `pip install -e ./apps/api`, then run the API with
`PYTHONPATH=apps/api uvicorn app.main:app --reload`. In another terminal run
...
```bash
pip install -e './apps/api[proctoring]'
PYTHONPATH=apps/api alembic -c apps/api/alembic.ini upgrade head
...
`VITE_FACE_DETECTOR_MODEL_URL` in `apps/candidate-web/.env.local` so monitoring does not depend on a public
