...
const ProfileScreen = ({ onSubmit, initialAlias = "" }: { onSubmit: (alias: string, resume: string) => Promise<void>; initialAlias?: string }) => {
  ...
  return <main className="preflight"><span className="interview-header__eyebrow">NAPOLEON [IT] · ASYNC INTERVIEW</span><h1>Перед началом интервью</h1><p>Введите имя для отображения в результатах и добавьте текст резюме. Эти данные будут использоваться только в рамках этого приглашения.</p><form onSubmit={(event) => void submit(event)}><label htmlFor="candidate-alias">Имя или псевдоним</label><input id="candidate-alias" value={alias} onChange={(event) => setAlias(event.target.value)} maxLength={120} required /><label htmlFor="candidate-resume">Резюме</label><textarea id="candidate-resume" value={resume} onChange={(event) => setResume(event.target.value)} maxLength={20000} rows={10} required placeholder="Вставьте текст резюме" />{message && <p role="alert">{message}</p>}<button disabled={saving}>{saving ? "Сохраняем…" : "Продолжить"}</button></form></main>;
};
