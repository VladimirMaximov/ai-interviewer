...
1. Apply migrations: `PYTHONPATH=backend python -m alembic -c backend/alembic.ini upgrade head`.
...
- `PYTHONPATH=backend python -m unittest discover -s backend/tests -v` — 53 tests passed.
- `PYTHONPATH=backend python -m compileall -q backend` — passed.
